...
class Main extends PluginBase implements Listener {
    ...
    public function clickEvent(Player $player, Inventory $inventory, Item $source, Item $target, int $slot) {
        ...
        if ($source->getName() == "§l§eQuest 1\n§r\nHancurkan Block Stone 256 Kali\nHadiah: 1 Juta") {
        ...
        } else if ($source->getName() == "§l§eQuest 2\n§r\nHancurkan Block Netherrack 256 Kali\nHadiah: 1,5 Juta") {
            $this->cQ->set($p, "2");
            $this->cQ->save();
            ...
            $this->progress->save();
            $player->removeWindow($inventory);
        } else if ($source->getName() == "§l§eQuest 3\n§r\nHancurkan Block Kayu 128 Kali\nHadiah: 2,5 Juta") {
            $this->cQ->set($p, "3");
            $this->cQ->save();
            ...
            $this->progress->save();
            $player->removeWindow($inventory);
        } else if ($source->getName() == "§l§eQuest 4\n§r\nPlace Block Kayu 150 Kali\nHadiah: 2,5 Juta") {
            $this->cQ->set($p, "4");
            $this->cQ->save();
            ...
            $this->progress->save();
            $player->removeWindow($inventory);
        } else if ($source->getName() == "§l§eQuest 5\n§r\nPlace Block Coblestone 300 Kali\nHadiah: 3,2 Juta") {
            $this->cQ->set($p, "5");
            $this->cQ->save();
        ...
        if ($this->checkQuest($p) == 1) {
            ...
            if ($block->getId() == 17) {
                ...
                if ($this->progress->get($p) == 128) {
                    ...
                    $this->rajin->set($p, $this->rajinCount($p) + 1);
                    $this->rajin->save();
                    $this->getServer()->broadcastMessage("§l§eSansQuest §g> §r" . $player->getName() . " Telah menyelesaikan §aQuest 3 §fDan mendapatkan Uang 2,5 Juta");
                }
            }
        }
    }
        ...
        if ($this->checkQuest($p) == 4) {
            if ($block->getId() == 17) {
                ...
                if ($this->progress->get($p) == 150) {
                    ...
                    $this->rajin->set($p, $this->rajinCount($p) + 1);
                    $this->rajin->save();
                    $this->getServer()->broadcastMessage("§l§eSansQuest §g> §r" . $player->getName() . " Telah menyelesaikan §aQuest 4 §fDan mendapatkan Uang 2,5 Juta");
                }
            }
            ...
            if ($block->getId() == 4) {
                ...
                if ($this->progress->get($p) == 300) {
                    ...
                    $this->rajin->set($p, $this->rajinCount($p) + 1);
                    $this->rajin->save();
                    $this->getServer()->broadcastMessage("§l§eSansQuest §g> §r" . $player->getName() . " Telah menyelesaikan §aQuest 5 §fDan mendapatkan Uang 3,2 Juta");
                }
            }
        }
    }
